...
public class GameTest extends TestBase {
    ...
    public void testPersistSuccess() {
        ...
        game.setExclusivity(Xclusivity.PLAYSTATION);
        ...
        //assertEquals(game.getPlatforms().size(), 1);
        assertEquals(game.getGenres().size(), 2);
        assertEquals(game.getDevelopers().size(), 1);
        assertEquals(game.getPublishers().size(), 1);
        assertEquals(game.getReviews().size(), 2);

        game.getDevelopers().forEach(dev -> {
            assertEquals(game.getName(), dev.getGames().get(0).getName());
        });
        game.getPublishers().forEach(pub -> {
            assertEquals(game.getName(), pub.getGames().get(0).getName());
        });
        game.getReviews().forEach(review -> {
            assertEquals(game.getName(), review.getGame().getName());
        });
